...
toString = additive
    -- Hier könnte man genauso gut die einfache Variante nehmen,
    -- die alles mit Klammern ausgibt.
    where
    additive (Put x y)  = additive x ++ " + " ++ multiplicative y
    additive (Take x y) = additive x ++ " - " ++ multiplicative y
    additive command    = multiplicative command

    multiplicative (Win x y) = multiplicative x ++ " * " ++ atomic y
    multiplicative command = unary command

    unary (Sig x) = "signum " ++ atomic x
    unary command = atomic command

    atomic (Val n) = show n
    atomic command = "(" ++ additive command ++ ")"
